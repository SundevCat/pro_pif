                    ...
                    <tbody>
                        <tr>
                            <td style="padding: 5px;">
                                {{ Form::select('list_sku[]',$list_sku,$data_sku, ['id'=>'list_sku[]','class'=>'custom-select2 form-control','multiple'=>'multiple']) }}
                            </td>
                            <td style="padding: 5px;"> 
                                {{ Form::select('list_product_year',$list_product_year,$data_product_year, ['id'=>'list_product_year','class'=>'custom-select2 form-control']) }}
                            </td>
                            <td style="padding: 5px;"> 
                                {{ Form::select('list_category',$list_category,$data_category, ['id'=>'list_category','class'=>'custom-select2 form-control']) }}
                            </td>
                            <td style="padding: 5px;"> 
                                {{ Form::select('list_catalog_status',$list_catalog_status,$data_catalog_status, ['id'=>'list_catalog_status','class'=>'custom-select2 form-control']) }}
                            </td>
                            <td style="padding: 5px;"> 
                                {{ Form::select('list_age[]',$list_age,$data_age, ['id'=>'list_age[]','class'=>'custom-select2 form-control','multiple'=>'multiple']) }}
                            </td>
                            <td style="padding: 5px;"> 
                                {{ Form::text('list_award',$data_award, ['id'=>'list_award','class'=>'form-control', 'placeholder'=> 'Award']) }}
                            </td>
                        </tr>
                    </tbody>
                </table>                
            ...
            <div class="row" style="overflow-x:auto;">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            @foreach($header_data as $value2)
                                <?php $lower_case = strtolower($value2); ?>
                                    <td>
                                        @if($lower_case == "main_image" ||
                                            $lower_case == "product_img_1" ||
                                            $lower_case == "product_img_2" ||
                                            $lower_case == "product_img_3" ||
                                            $lower_case == "product_img_4" ||
                                            $lower_case == "product_img_5" ||
                                            $lower_case == "product_img_6" ||
                                            $lower_case == "product_img_7" ||
                                            $lower_case == "product_img_8" ||
                                            $lower_case == "product_img_9" ||
                                            $lower_case == "package_shot_1" ||
                                            $lower_case == "lifestyle_img_1" ||
                                            $lower_case == "lifestyle_img_2" ||
                                            $lower_case == "lifestyle_img_3" ||
                                            $lower_case == "lifestyle_img_4" ||
                                            $lower_case == "lifestyle_img_5" ||
                                            $lower_case == "lifestyle_img_6" ||
                                            $lower_case == "lifestyle_img_7" ||
                                            $lower_case == "lifestyle_img_8" ||
                                            $lower_case == "lifestyle_img_9" ||
                                            $lower_case == "lifestyle_img_10" ||
                                            $lower_case == "package_shot_2" ||
                                            $lower_case == "package_shot_3" ||
                                            $lower_case == "package_shot_4"

                                        )
                                            @if($value->$lower_case != "N/A") 
                                                <img src="{{ $value->$lower_case }}" style="width: 100px;height: 100px;">
                                            @else
                                                <img src="{{ url('/public/images/Image-Not-Available.png') }}" style="width: 80px;height: 100px;">
                                            @endif
                                        @else
                                            {{ $value->$lower_case }}
                                        @endif
                ...
                <?php 
                    // print_r($data_sku); die;
                    // $data_sku = "9982,8709";
                    if(is_array($data_sku)){
                        $data_sku = implode(",",$data_sku);
                    }

                    if(is_array($data_age)){
                        $data_age = implode(",",$data_age);
                    }
...
<script src="{{ url('/public/src/plugins/datatables/js/jquery.dataTables.min.js') }}"></script>
    ...
    <script type="text/javascript">
        ...
        $(document).ready( function () {
            ...
            input = document.querySelector('#list_fillter');
            input2 = document.querySelector('#orderby_data');
            container = document.querySelector('.tag-container');
            hashtagArray = [];

            $('#btn_submit_fillter').click(function (event) {
                ...
                if(hashtagArray.length > 0){
                    ...
                    const index = hashtagArray.indexOf(data);
                    ...
                    if (index > -1) {
                        ...
                        var text = document.createTextNode(input.value+"|"+input2.value);
                        
                        p.appendChild(text);
                        p.classList.add('tag_data');
                        // p.style.display ="inline-block";
                        p.onclick = function(){ del(this) };
                        input.value = '';
                ...
                $("#fillter_input").val( );
                $("#fillter_input").val(hashtagArray);
